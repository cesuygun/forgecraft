...
import type { GenerationRequest, QueueItem, QueueStatus, QueueStatusMessage } from "../../shared/types";
...
export const getQueueItem = (id: string): QueueItem | null => {
	const db = getDatabase();
	const row = db.prepare("SELECT * FROM generation_queue WHERE id = ?").get(id) as
		| QueueRow
		| undefined;
...
export const deleteQueueItem = (id: string): boolean => {
	const db = getDatabase();
	const result = db.prepare("DELETE FROM generation_queue WHERE id = ?").run(id);
...
export const getQueueStatus = (): QueueStatusMessage => {
	...
	const generatingItem = db
		.prepare("SELECT id FROM generation_queue WHERE status = 'generating' LIMIT 1")
...
export const clearCompletedQueue = (): number => {
	const db = getDatabase();
	const result = db.prepare("DELETE FROM generation_queue WHERE status = 'complete'").run();
